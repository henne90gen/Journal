...
class JournalViewListener implements ListSelectionListener, ActionListener, DocumentListener {
    ...
    JournalViewListener(Journal journal) { this.journal = journal; }
    ...
    public void actionPerformed(ActionEvent e) {
        switch (e.getActionCommand()) {
            case NEW_BUTTON:
                ...
                journal.view.update();
                journal.view.dateList.setSelectedIndex(journal.data.entries.size()-1);
                actionPerformed(new ActionEvent(this, 0, EDIT_BUTTON));
                break;
            case EDIT_BUTTON:
                System.out.println("Editing entry " + journal.view.dateList.getSelectedValue().getID() + ".");
    ...
    private void searchDate() {
        ...
        Entry[] es = journal.data.searchForDate(date[1], date[0], date[2]);
        journal.view.dateList.setListData(es);
        if (es.length == 0) {
    ...
    private void searchString() {
        journal.view.dateList.setListData(journal.data.searchForString(journal.view.searchTF.getText()));
        journal.view.dateList.setSelectedIndex(0);
    }
